...
import com.fijo.ebox.enums.ModularEnum.SexEnum;
 ...
 **/
@RestController
@RequestMapping("/BXFA0001")
@Slf4j
public class BXFA0001Controller {
	@Autowired
	private BXFA0001Service bxfa0001Service;
	...
	public String queryAll(String orgCode){
		List<BXFA0001> resultList = bxfa0001Service.queryAll();
	...
	public String queryAllByPage(String orgCode,Integer pageNo,Integer pageSize){
		try {
			Map result =  bxfa0001Service.queryAllByPage(orgCode,pageNo,pageSize);
	...
	public String queryById(Long id){
		BXFA0001 bxfa0001 = bxfa0001Service.queryById(id);
		return ResultDto.SUCCESS(bxfa0001);
	}
	/**
	* 新增
	* @param bxfa0001
	...
	*/
	...
	public String insert(BXFA0001 bxfa0001){
		try {
			bxfa0001Service.insert(bxfa0001);
			log.info("【新增xxx】 新增成功!");
	...
	*/
	...
	public String update(BXFA0001 bxfa0001){
		try {
			bxfa0001Service.update(bxfa0001);
			log.info("【更新xxx】 更新成功!");
	...
	*/
	...
	public String logicDeleteById(String id){
		try {
			bxfa0001Service.logicDeleteById(id);
			log.info("【删除xxx】 删除成功!");
			return ResultDto.SUCCESS("删除成功！");
		} catch (Exception e) {
			log.error("【删除xxx】 删除失败，失败原因：{}",e.toString());
	...
	public String exportTable(String queryStr,String columnList,HttpServletResponse response){
		try {
			BXFA0001 bxfa0001 =  new BXFA0001();
			bxfa0001 = JsonUtil.json2Bean(queryStr,BXFA0001.class);
			String sheetName = "FA基本信息";
			...
			List<BXFA0001> resultList = new ArrayList();
			resultList = bxfa0001Service.queryAll();
			...
			outPutConfig.put("sax",EnumUtil.getEnumMap(SexEnum.class));//获取枚举的key，value 类型的map
			//如果不是枚举类型，可以自定义转换配置，
	...
	public String downloadTemplate(HttpServletResponse response){
		...
		try {
			List<String> ignoreFieldList = Arrays.asList("id","sort","remark","col1","col2","col3","col4","col5","col6","col7","col8","col9","col10","distributeResultStr");//需要忽略的字段
			Map<String,String[]> fieldMap = PlatPojoUtil.getClazzFieldsInfo(BXFA0001.class,ignoreFieldList);//TODO
			...
			filedOptionConfig.put("sax",EnumUtil.getEnumValue(SexEnum.class));
			//获取处理好的工作薄对象，为导出做准备
	...
	public String queryTemplateFields() {
		List<String> ignoreFieldList = Arrays.asList("id","sort","remark","distributeResultStr");//需要忽略的字段
		Map<String,String[]> fieldMap = null;
		List<Map> resultList = new ArrayList<>();
		try {
			fieldMap = PlatPojoUtil.getClazzFieldsInfo(BXFA0001.class,ignoreFieldList);
			String [] columnArr_field = fieldMap.get(PlatPojoUtil.FIELD_EN_KEY);
			String [] columnArr_title = fieldMap.get(PlatPojoUtil.FIELD_CN_KEY);
	...
	public String importFile( MultipartFile file) {
			...
			List<BXFA0001> errorList = new ArrayList<>();
			try {
				//1.定义枚举类配置
				Map<String,Class> enumConfig = new HashMap<>();
				enumConfig.put("sax", SexEnum.class);//定义枚举类型的字段，map中的key为字段名，value为枚举对应的枚举类 枚举类中必须定义getEnumCodeByMsg方法
				List resultList = PlatExeclUtil.readExecl(file,BXFA0001.class,0,1,3,enumConfig);
				resultList.stream().forEach( t ->{
					BXFA0001 bxfa0001 = (BXFA0001) t;
					try {
						bxfa0001Service.insert(bxfa0001);
					} catch (Exception e) {
						errorList.add(bxfa0001);
						e.printStackTrace();
					}
				});
			} catch (Exception e) {
