		...
		methods: {
			batchCancelDistribute(){
				...
				if(allSelectedList == null || allSelectedList.length == 0) {
					this.$message.warning('请选择需要取消分配的数据！');
					return false;
				} else {
					let ids = "";
					...
					this.$confirm('此操作将取消'+allSelectedList.length+'条保单的分配信息, 是否继续?', '提示', {
						confirmButtonText: '确定',
						cancelButtonText: '取消',
						type: 'warning'
					}).then(() => {
						batchCancelDistribute(ids); //执行删除
					}).catch(() => {
						this.$message({
							type: 'info',
							message: '已取消'
			...
			$.each(cols_table, function(index, val) {
				...
				if(val.formatter == "f_isDistributeFa"){
					val.formatter = function(value, row, i) {
						...
						return objJson == null? null :objJson.label
					}
				}
...
function batchCancelDistribute(ids) {
	let params = {};
	params.ids = ids;
	params.isDistributeFa = false;
